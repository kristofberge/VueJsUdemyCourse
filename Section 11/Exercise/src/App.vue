<template>
  <div class="container">
    <form v-if="!isSubmitted">
      <div class="row">
        <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
          <!-- Exercise 1 -->
          <!-- Create a Signup Form where you retrieve the following Information -->
          <!-- Full Name (First Name + Last Name) -->
          <!-- Mail -->
          <!-- Password -->
          <!-- Store Data? Yes/No -->
            <fullname-input v-model="fullName"></fullname-input>
          <div class="form-group">
            <label for="email">E-mail</label>
            <input id="email" type="text" v-model.lazy="email" />
          </div>
          <div class="form-group">
            <label for="password">Password</label>
            <input id="password" type="password" v-model.lazy="password" />
          </div>
          <div class="form-group">
            <label>Store data in database?</label>
            <br />
            <label>
              <input type="radio" value="Yes" v-model="storeData" /> Yes
            </label>
            <label>
              <input type="radio" value="No" v-model="storeData" /> No
            </label>
            <!-- Exercise 2 -->
            <!-- Only display the Form if it has NOT been submitted -->
            <!-- Display the Data Summary ONCE the Form HAS been submitted -->
          </div>
          <button class="btn btn-primary" @click.prevent="submitForm">Submit!</button>
          <!-- Exercise 3 -->
          <!-- Edit the Example from above and create a custom "Full Name" Control -->
          <!-- which still holds the First Name and Last Name Input Field -->
        </div>
      </div>
    </form>
    <hr />
    <div class="row" v-if="isSubmitted">
      <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
        <div class="panel panel-default">
          <div class="panel-heading">
            <h4>Your Data</h4>
          </div>
          <div class="panel-body">
            <p>Full Name: {{fullName}}</p>
            <p>Mail: {{email}}</p>
            <p>Password: {{password}}</p>
            <p>Store in Database?: {{storeData}}</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import FullnameInput from "./FullnameInput.vue";

export default {
  components: {
      fullnameInput: FullnameInput
  },
  data() {
    return {
      fullName: "",
      email: "",
      password: "",
      storeData: "No",
      isSubmitted: false
    };
  },
  methods: {
    submitForm() {
      this.isSubmitted = true;
    }
  }
};
</script>

<style>
</style>
